...
type DeviceSessionRevocationRow = {
...
describe('SessionService', () => {
  it('rotates refresh tokens and updates session metadata', async () => {
    ...
    try {
      ...
      const accessClaims = jwt.verify(response.accessToken, env.jwtSecret) as jwt.JwtPayload & {
        deviceId: string;
      ...
      const refreshClaims = jwt.verify(response.refreshToken, env.jwtSecret) as jwt.JwtPayload & {
        deviceId: string;
        tokenId: string;
        rotationId: string;
        sub: string;
      ...
      const row = await db.pool.query<DeviceSessionMetaRow>(
        `
  ...
  it('loads device session context from access tokens', async () => {
    ...
    try {
      ...
      const row = await db.pool.query<DeviceSessionSeenRow>(
        `
          SELECT last_seen_at
          FROM device_sessions
          WHERE id = $1
