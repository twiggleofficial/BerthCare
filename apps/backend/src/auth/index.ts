...
type SessionRevokeBody = SessionRefreshBody & {
  reason?: string;
};
...
export const createAuthRouter = (): Router => {
  ...
  router.post('/login', async (req, res) => {
    const validation = loginSchema.validate(req.body, {
      abortEarly: false,
      stripUnknown: true,
    }) as Joi.ValidationResult<LoginPayload>;
...
export const createAuthV1Router = (options: CreateAuthV1RouterOptions = {}): Router => {
  const router = Router();
  ...
  router.post('/activate', async (req, res, next) => {
    ...
    const validation = activationSchema.validate(req.body, {
      abortEarly: false,
      stripUnknown: true,
    }) as Joi.ValidationResult<ActivationRequestBody>;
    const requestId = typeof res.locals.requestId === 'string' ? res.locals.requestId : undefined;
  ...
  router.post('/activate/complete', async (req, res, next) => {
    ...
    const validation = activationCompletionSchema.validate(req.body, {
      abortEarly: false,
      stripUnknown: true,
    }) as Joi.ValidationResult<ActivationCompletionBody>;
    const requestId = typeof res.locals.requestId === 'string' ? res.locals.requestId : undefined;
  ...
  router.post('/session/refresh', async (req, res, next) => {
    ...
    const validation = sessionRefreshSchema.validate(req.body, {
      abortEarly: false,
      stripUnknown: true,
    }) as Joi.ValidationResult<SessionRefreshBody>;
    const requestId = typeof res.locals.requestId === 'string' ? res.locals.requestId : undefined;
