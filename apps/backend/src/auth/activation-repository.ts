...
export const createActivationRepository = (): ActivationRepository => {
  return {
    async findUserByEmail(client, email) {
      ...
      const result = await client.query<ActivationUserRecord>(query, [email]);
    ...
    async countRecentAttempts(client, email, deviceFingerprint, windowMinutes) {
      ...
      const result = await client.query<{ attempts: number }>(query, [
        email,
        deviceFingerprint,
        windowMinutes,
      ]);
    ...
    async findActivationSessionByTokenHash(client, activationTokenHash) {
      type ActivationSessionRow = {
        ...
        revokedAt: Date | null;
        userEmail: string;
        userActivationCodeHash: string | null;
        userActivationExpiresAt: Date | null;
        userFirstName: string;
        userLastName: string;
        userRole: ActivationUserRecord['role'];
      ...
      const result = await client.query<ActivationSessionRow>(query, [activationTokenHash]);
      ...
      const user: ActivationUserRecord = {
        id: row.userId,
        email: row.userEmail,
