...
type ActivationAttemptRow = {
  success: boolean;
  outcome: string;
};

type ActivationSessionFingerprintRow = {
...
type DeviceSessionRow = {
  ...
  pin_scrypt_hash: string;
  pin_scrypt_salt: string;
  pin_scrypt_params: string;
...
type ActivationSessionCompletionRow = {
  completed_at: Date | null;
};

type ActivationSessionIdRow = {
  id: string;
};
...
describe('ActivationService', () => {
  it('issues activation token for valid caregiver credentials', async () => {
    ...
    try {
      ...
      const attempts = await db.pool.query<ActivationAttemptRow>(
        'SELECT success, outcome FROM auth_activation_attempts ORDER BY created_at ASC',
      );
      expect(attempts.rowCount).toBe(1);
      expect(attempts.rows[0].success).toBe(true);

      const sessions = await db.pool.query<ActivationSessionFingerprintRow>(
        'SELECT device_fingerprint FROM auth_activation_sessions',
      );
  ...
  it('completes activation and creates a device session with tokens', async () => {
    ...
    try {
      ...
      expect(completion.deviceId).toMatch(
        /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i,
      );

      const accessPayload = jwt.verify(completion.accessToken, env.jwtSecret) as jwt.JwtPayload & {
        deviceId: string;
      ...
      const activationSession = await db.pool.query<ActivationSessionCompletionRow>(
        `
          SELECT completed_at
  ...
  it('rejects activation completion when device already enrolled', async () => {
    ...
    try {
      ...
      const activationSession = await db.pool.query<ActivationSessionIdRow>(
        `
          SELECT id
          FROM auth_activation_sessions
          WHERE user_id = $1
