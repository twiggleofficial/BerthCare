...
import { authorize, canAccessZone, hasPermission, hasRole, loadDeviceSession, } from './middleware.js';
...
const createRequest = (overrides = {}) => {
    ...
    const request = {
        ...
        body: {},
        headers: baseHeaders,
        get: (header) => baseHeaders[header.toLowerCase()],
        ...overrides,
    };
    return request;
};
...
const expectErrorResponse = (res, code, message) => {
...
describe('hasPermission', () => {
    ...
    it('supports any-match semantics', () => {
        const user = createUser({ role: 'caregiver' });
        expect(hasPermission(user, ['write:care_plans', 'create:visits'], {
            match: 'any',
        })).toBe(true);
    });
...
describe('authorize middleware', () => {
    ...
    it('allows custom zone resolver', () => {
        const middleware = authorize({
            zone: {
                resolver: (req) => req.headers?.['x-zone-key'] ?? null,
            },
        });
        const req = createRequest({
