...
| B8 | Implement error handling middleware | Update `/apps/backend/src/app.ts` to expose centralized error handling with consistent response format (code, message, details, timestamp, requestId). Implement error codes from project-documentation/architecture-output.md (AUTH_*, VALIDATION_*, RESOURCE_*, CONFLICT_*, SERVER_*). Add request ID generation and logging. Cite project-documentation/architecture-output.md – Error Response Format. Philosophy: deliver lean, predictable services and obsess over unseen details. | B7 | Error handler in `app.ts`; error code constants | All errors return consistent format; error codes match spec; request IDs logged; tests cover all error types | Backend Dev | 1d |
