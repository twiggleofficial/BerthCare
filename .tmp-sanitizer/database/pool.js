...
exports.closePool = exports.checkDatabaseConnection = exports.runWithClient = exports.getReadReplicaPool = exports.getPool = void 0;
...
const buildPoolConfig = (role) => {
    ...
    const host = (_a = readEnv(role, 'HOST', DEFAULT_DB_HOST)) !== null && _a !== void 0 ? _a : DEFAULT_DB_HOST;
    const port = (0, parse_1.parseInteger)(readEnv(role, 'PORT'), DEFAULT_DB_PORT, { min: 1, max: 65535 });
    const database = (_b = readEnv(role, 'NAME', DEFAULT_DB_NAME)) !== null && _b !== void 0 ? _b : DEFAULT_DB_NAME;
    const user = (_c = readEnv(role, 'USER', DEFAULT_DB_USER)) !== null && _c !== void 0 ? _c : DEFAULT_DB_USER;
    const password = readEnv(role, 'PASSWORD', process.env.DB_PASSWORD);
    const maxConnections = (0, parse_1.parseInteger)(readEnv(role, 'POOL_MAX'), MAX_POOL_CONNECTIONS, {
        min: 1,
        max: MAX_POOL_CONNECTIONS,
    });
    const idleTimeoutMillis = (0, parse_1.parseInteger)(readEnv(role, 'IDLE_TIMEOUT_MS'), DEFAULT_IDLE_TIMEOUT_MS, { min: 1 });
    const connectionTimeoutMillis = (0, parse_1.parseInteger)(readEnv(role, 'CONNECTION_TIMEOUT_MS'), DEFAULT_CONNECTION_TIMEOUT_MS, { min: 1 });
    const applicationNameDefault = role === 'read-replica'
        ? `${(_d = process.env.DB_APPLICATION_NAME) !== null && _d !== void 0 ? _d : 'berth-backend'}-read`
        : ((_e = process.env.DB_APPLICATION_NAME) !== null && _e !== void 0 ? _e : 'berth-backend');
    const applicationName = (_f = readEnv(role, 'APPLICATION_NAME', applicationNameDefault)) !== null && _f !== void 0 ? _f : applicationNameDefault;
...
let readReplicaPool;
const shouldInitialiseReadReplica = () => Boolean(process.env.DB_REPLICA_HOST || process.env.DB_REPLICA_NAME || process.env.DB_REPLICA_USER);
if (shouldInitialiseReadReplica()) {
    try {
        ...
        const err = error;
        logger_1.logger.warn('Failed to initialise read replica pool, falling back to primary for reads', {
            message: err.message,
        });
    ...
    logger_1.logger.info('Read replica pool not configured; primary pool will handle read operations');
}
...
const checkDatabaseConnection = async () => {
    ...
    return {
        healthy: primaryHealth.healthy && ((_a = replicaHealth === null || replicaHealth === void 0 ? void 0 : replicaHealth.healthy) !== null && _a !== void 0 ? _a : true),
        nodes: {
