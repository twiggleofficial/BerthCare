...
exports.authRouter =
  exports.logoutRateLimiter =
  exports.refreshRateLimiter =
  exports.loginRateLimiter =
  exports.registerRateLimiter =
    void 0;
...
const hashRefreshToken = (token) =>
  node_crypto_1.default.createHash('sha256').update(token).digest('hex');
...
const rateLimitKeyGenerator = (req) => {
  ...
  const userId =
    req.user && typeof req.user.id === 'string' && req.user.id.trim().length > 0
      ? req.user.id.trim()
      : null;
...
const isUniqueViolation = (error) =>
  Boolean(error && typeof error === 'object' && error.code === UNIQUE_VIOLATION_CODE);
...
exports.logoutRateLimiter = createRateLimiter(
  logoutRateMax,
  'Too many requests. Please try again later.'
);
...
exports.authRouter.post('/register', exports.registerRateLimiter, async (req, res, next) => {
  var _a;
  const validation = (0, validation_1.sanitiseRegistrationPayload)(
...
exports.authRouter.post('/refresh', exports.refreshRateLimiter, async (req, res, next) => {
  var _a;
  const validation = (0, validation_1.sanitiseRefreshPayload)(
    (_a = req.body) !== null && _a !== void 0 ? _a : {}
  );
  if (!validation.ok) {
    res.status(400).json(formatValidationErrorResponse(validation.errors));
  ...
  const userId = typeof user_id === 'string' ? user_id : typeof sub === 'string' ? sub : undefined;
...
exports.authRouter.post('/login', exports.loginRateLimiter, async (req, res, next) => {
  var _a;
  const validation = (0, validation_1.sanitiseLoginPayload)(
    (_a = req.body) !== null && _a !== void 0 ? _a : {}
  );
  if (!validation.ok) {
    res.status(400).json(formatValidationErrorResponse(validation.errors));
  ...
  try {
    const result = await (0, pool_1.runWithClient)(async (client) => {
      ...
      const passwordHash =
        (_a = row === null || row === void 0 ? void 0 : row.password_hash) !== null && _a !== void 0
          ? _a
          : '';
      const passwordValid = await (0, auth_utils_1.verifyPassword)(
...
exports.authRouter.post(
  ...
  async (req, res, next) => {
    var _a;
    const validation = (0, validation_1.sanitiseLogoutPayload)(
      (_a = req.body) !== null && _a !== void 0 ? _a : {}
    );
    if (!validation.ok) {
      res.status(400).json(formatValidationErrorResponse(validation.errors));
