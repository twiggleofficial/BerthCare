...
exports.closeRedisConnections =
  ...
  exports.getSessionClient =
  exports.getCacheClient =
...
const shouldUseTls = () => {
  var _a;
  const raw =
    (_a = process.env.REDIS_USE_TLS) !== null && _a !== void 0 ? _a : process.env.REDIS_TLS;
...
const createRetryStrategy = () => {
  const baseDelayMs = (0, parse_1.parseInteger)(
    process.env.REDIS_RETRY_BASE_MS,
    DEFAULT_RETRY_BASE_DELAY_MS,
    {
      min: 10,
    }
  );
  const maxDelayMs = (0, parse_1.parseInteger)(
    process.env.REDIS_RETRY_MAX_MS,
...
const buildRedisOptions = (keyPrefix) => {
  ...
  const port = (0, parse_1.parseInteger)(process.env.REDIS_PORT, DEFAULT_REDIS_PORT, {
    min: 1,
    max: 65535,
  });
  const db = (0, parse_1.parseInteger)(process.env.REDIS_DB, DEFAULT_REDIS_DB, { min: 0 });
  const password = process.env.REDIS_PASSWORD;
  const connectTimeout = (0, parse_1.parseInteger)(
  ...
  const maxRetriesPerRequest = (0, parse_1.parseInteger)(
    process.env.REDIS_MAX_RETRIES_PER_REQUEST,
    DEFAULT_MAX_RETRIES_PER_REQUEST,
    { min: 0 }
  );
  const enableOfflineQueue = (0, parse_1.parseBoolean)(
    process.env.REDIS_ENABLE_OFFLINE_QUEUE,
    false
  );
  const connectionName =
    (_b = process.env.REDIS_CONNECTION_NAME) !== null && _b !== void 0
      ? _b
...
const createRedisClient = (role, keyPrefix) => {
  ...
  if (url && !hasLoggedRedisUrlOverrideWarning) {
    const conflicting = INDIVIDUAL_REDIS_ENV_VARS.filter(
      (name) => typeof process.env[name] === 'string'
    );
    if (conflicting.length > 0) {
      logger_1.logger.warn(
        'REDIS_URL is set; overriding individual Redis configuration variables',
        {
...
const getCacheClient = () => {
  ...
  if (!cacheClient) {
    const prefix =
      (_a = process.env.REDIS_CACHE_PREFIX) !== null && _a !== void 0 ? _a : DEFAULT_CACHE_PREFIX;
...
const getSessionClient = () => {
  ...
  if (!sessionClient) {
    const prefix =
      (_a = process.env.REDIS_SESSION_PREFIX) !== null && _a !== void 0
        ? _a
