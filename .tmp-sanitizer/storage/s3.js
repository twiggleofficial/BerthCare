...
exports.resetS3Client =
  exports.checkPhotoBucketHealth =
  exports.generatePhotoUploadUrl =
  exports.buildPhotoStorageKey =
  exports.getS3Client =
    void 0;
...
const resolveRegion = () => {
  var _a;
  return (_a = process.env.AWS_REGION) !== null && _a !== void 0 ? _a : DEFAULT_REGION;
};
const resolveBucket = (override) => {
  var _a, _b;
  return (_b =
    (_a = override !== null && override !== void 0 ? override : process.env.AWS_S3_PHOTO_BUCKET) !==
      null && _a !== void 0
      ? _a
      : process.env.AWS_S3_BUCKET) !== null && _b !== void 0
    ? _b
    : DEFAULT_BUCKET;
};
const resolvePresignedTtl = (override) => {
  ...
  return (0, parse_1.parseInteger)(
    process.env.S3_PRESIGNED_TTL_SECONDS,
...
const buildS3Config = () => {
  ...
  const requestTimeout = (0, parse_1.parseInteger)(
  ...
  if (!process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY) {
    logger_1.logger.warn(
      'AWS credentials not found in environment; relying on default provider chain'
    );
  }
  return config;
};
...
const buildPhotoStorageKey = ({ visitId, extension, capturedAt }) => {
  ...
  if (capturedAt instanceof Date || typeof capturedAt === 'string') {
    ...
    if (Number.isNaN(candidate.getTime())) {
      logger_1.logger.warn(
        'Invalid capturedAt provided for photo key; falling back to current time',
        {
          capturedAt,
          visitId: safeVisitId,
        }
...
const generatePhotoUploadUrl = async ({
  fileName,
  contentType,
  visitId,
  caregiverId,
